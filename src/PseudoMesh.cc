...
void hex_indices_by_pos(G4double x, G4double y, G4double a, G4int iX_max, G4int iY_max, G4int *x_i, G4int *y_i)//x,y from [0,0] to [Xmax, Ymax]
{
	...
	*y_i = -1;
	...
	for (int iix = -1, int iiy = -1; ((iix < 2) && (iiy < 2)); iix++, iiy++) //finding the exact center (x,y) in in the hexagonal
	{
...
void hex_on_leave (G4ThreeVector GlobalParentPos /*center*/, G4double x_par, G4double y_par,
	...
	G4double active_x_par, G4double active_y_par)
{
	...
	*curr_x_id = -1;
	*curr_y_id = -1;
	step->GetPostStepPoint()->SetPosition(new_pos);
	return;
}
...
void hexagonal_mapping(G4ThreeVector GlobalParentPos /*center*/, G4double x_par, G4double y_par, G4double z_par,
	G4ThreeVector GlobalCellPos, G4double cell_size, G4int *curr_x_id, G4int *curr_y_id, const G4Step* step)
{
	G4int X_max_index = int (x_par/(2*cell_size));
	G4int Y_max_index = int((y_par - cell_size / (sqrt(3))) / (cell_size*sqrt(3)));
	if (X_max_index <= 0 || Y_max_index <= 0) return; //no grid - no changes
	...
	if ((*curr_x_id < 0)||(*curr_y_id<0))//photon from outside
	{
		...
		if ((abs((abs(dz) - z_par / 2)) < tolerance) && (G4ThreeVector(0, 0, dz)*dir < 0)) //on the surface and moves inside
		{
			...
			*curr_y_id = y_i;
			...
			return; //no changes are proposed
	}
	else //photon is already in the cell
	{
		...
		{
			hex_on_leave(GlobalParentPos, x_par, y_par,
				GlobalCellPos, cell_size, curr_x_id, curr_y_id, step,active_x_par, active_y_par);
			return;
		}
		...
		if ((abs(cell_size - x_rel)<tolerance)&&(dir.x>0)) //leaving x+ edge
		{
			*curr_x_id++;
			...
			pos.x -= cell_size * 2;
			step->GetPostStepPoint()->SetPosition(pos);
			return;
		}
		...
		if ((abs(y_rel - cell_size*sqrt(3) +x_rel / sqrt(3))<tolerance)&&(dir*G4ThreeVector(0.5,sqrt(3)/2,0)>0)) //x+,y+
		{
			...
			*curr_y_id++;
			...
			pos.x -= 2 * cell_size*0.5;
			pos.y -= 2 * cell_size*sqrt(3) / 2;
			step->GetPostStepPoint()->SetPosition(pos);
			return;
		}
		if ((abs(y_rel - cell_size*sqrt(3) - x_rel / sqrt(3))<tolerance) && (dir*G4ThreeVector(-0.5, sqrt(3) / 2, 0)>0)) //x-,y+
...
void PseudoMesh::PostSpeppingAction(const G4Step* step) //checks entering/leaving, called from DetectorConstruction, manages everything
{
	...
	if ((step->GetTrack()->GetTrackStatus()==fGeomBoundary)&&((postV == parent) || (preV == cell)))
		map_function(g_parent_pos, par_x_size, par_y_size, par_z_size, g_cell_posit, cell_size, &curr_x_id, &curr_y_id, step);
}
